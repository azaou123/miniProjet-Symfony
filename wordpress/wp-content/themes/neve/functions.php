...
function creer_taxonomie_niveau_difficulte() {
    ...
    register_taxonomy(
        ...
        [
            'labels' => [
                'name'              => 'Niveaux de difficulté',
                'singular_name'     => 'Niveau de difficulté',
                'search_items'      => 'Rechercher un niveau',
                'all_items'         => 'Tous les niveaux',
                'edit_item'         => 'Modifier le niveau',
                'update_item'       => 'Mettre à jour le niveau',
                'add_new_item'      => 'Ajouter un nouveau niveau',
                'new_item_name'     => 'Nouveau niveau de difficulté',
                'menu_name'         => 'Niveau de difficulté',
            ],
            'show_in_rest'       => true,  // Pour l'éditeur Gutenberg
            'hierarchical'       => true,  // Fonctionne comme les catégories
            'show_admin_column'  => true,  // Affiche la colonne dans l'admin
            'public'             => true,  // Rend la taxonomie accessible
            'rewrite'            => [
                'slug' => 'niveau-difficulte', // URL personnalisée
                'with_front' => true,          // Préfixe d'URL
            ],
        ]
    );
}
add_action('init', 'creer_taxonomie_niveau_difficulte');
// Enregistrer la route API REST pour récupérer les articles par niveau de difficulté
function register_custom_api_routes() {
    register_rest_route( 'custom-api', '/posts-by-difficulty', array(
        'methods' => 'GET',
        'callback' => 'get_posts_by_difficulty',
        'args' => array(
            'difficulty' => array(
                'required' => true,
                'validate_callback' => function( $param, $request, $key ) {
                    // Validation : vérifier si le terme de difficulté existe
